...
Bigint one = convert(1) , zero = convert(0);

Bigint nCr(LLI n , LLI r)
{
    Bigint num = one,div = one;
    if(r == 0 ){
        return one;
    }
    for(int cont =0; cont < n-(n-r) ; cont++){
        num = num*convert(n-cont);
    }
    for(; r>1 ; r--){
        div =  div*convert(r);
    }
...
Bigint solve(int a,int b)
{
    Bigint tot = one , tmp = nCr(a,b);
    for(LLU i=0 ; i<b ; i++)
    {
        tot = tot*convert(a-i);
    }
